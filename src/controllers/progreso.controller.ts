...
export const firmarRequisito = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Extraemos todos los datos de la credencial (token) del evaluador
    const evaluadorId = req.usuario?.id;
    const rolEvaluador = req.usuario?.rol;
    const clubEvaluadorId = req.usuario?.clubId;
    ...
    // 🛡️ CORTAFUEGOS ANTI-IDOR: Aislar la jurisdicción
    const integranteDb = await prisma.integrante.findUnique({
      where: { id: Number(integranteId) },
      select: { clubId: true }, // Solo traemos lo que necesitamos para no cargar la memoria
    });

    if (!integranteDb)
      return res.status(404).json({
        status: "error",
        message: "Integrante no encontrado en la base de datos.",
      });

    // Si es un DIRECTOR, estrictamente debe ser del mismo club que el integrante
    if (
      rolEvaluador === "DIRECTOR" &&
      integranteDb.clubId !== clubEvaluadorId
    ) {
      console.warn(
        `🛑 INTRUSIÓN: Evaluador ${evaluadorId} intentó firmar a un integrante del club ${integranteDb.clubId}`,
      );
      return res.status(403).json({
        status: "error",
        message:
          "Brecha de seguridad: No tenés jurisdicción para evaluar a un integrante de otro club.",
      });
    }

    // Buscamos el requisito
    ...
    if (!requisitoDb)
      return res
        .status(404)
        .json({ status: "error", message: "Requisito no encontrado." });
    ...
    if (yaFirmado)
      return res.status(400).json({
        status: "error",
        message: "Requisito ya firmado anteriormente.",
      });
    ...
    if (requisitoDb.opcionesExtra) {
      if (requisitoDb.opcionesExtra.startsWith("FIJA:")) {
        ...
        requisitoDb.opcionesExtra.startsWith("OPCIONES:") ||
        requisitoDb.opcionesExtra.startsWith("ABIERTA:")
      ) {
        if (!especialidadElegidaId) {
          return res.status(400).json({
            status: "error",
            message: "Este requisito exige que envíes especialidadElegidaId.",
          });
        }

        const espElegida = await prisma.especialidad.findUnique({
          where: { id: Number(especialidadElegidaId) },
        });
        if (!espElegida)
          return res.status(404).json({
            status: "error",
            message: "La especialidad elegida no existe.",
          });

        if (requisitoDb.opcionesExtra.startsWith("OPCIONES:")) {
          const permitidas = requisitoDb.opcionesExtra
            .split(":")[1]
            .split("|")
          ...
          if (!permitidas.includes(espElegida.nombre.toLowerCase())) {
            return res.status(403).json({
              status: "error",
              message: `Especialidad no permitida. Opciones: ${permitidas.join(", ")}`,
            });
          }
        }

        if (requisitoDb.opcionesExtra.startsWith("ABIERTA:")) {
          const categoriaRequerida = requisitoDb.opcionesExtra
            .split(":")[1]
            .trim()
            .toLowerCase();
          if (espElegida.categoria.toLowerCase() !== categoriaRequerida) {
            return res.status(403).json({
              status: "error",
              message: `La especialidad debe ser de la categoría: ${categoriaRequerida}`,
            });
          }
        }
    ...
    if (aprobados >= totalRequisitos && totalRequisitos > 0) {
      ...
      mensajeExtra +=
        " ¡El integrante completó el 100% de la clase y está INVESTIDO!";
    }
    ...
    return res
      .status(500)
      .json({ status: "error", message: "Fallo al firmar." });
  }
};
...
export const obtenerProgreso = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (totalRequisitos === 0) {
      return res.status(400).json({
        status: "error",
        message: "La clase no tiene requisitos cargados.",
      });
    }
    ...
    return res.status(500).json({
      status: "error",
      message: "Fallo interno al calcular el progreso.",
    });
  }
};
// 3. BUSCAR REQUISITOS PENDIENTES (Para el Frontend)
export const obtenerRequisitosPendientes = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
    ...
    if (!integrante || !integrante.claseId) {
      return res
        .status(400)
        .json({ status: "error", message: "No tiene clase asignada." });
    }
      ...
      sec.requisitos.forEach((req) => {
        if (req.progresos.length === 0) {
          pendientes.push({
            ...
            esEspecialidad:
              req.descripcion.toLowerCase().includes("especialidad") ||
              sec.titulo.toLowerCase().includes("especialidad"),
    ...
    return res
      .status(500)
      .json({ status: "error", message: "Fallo al buscar pendientes." });
  }
};

// 4. NUEVO: ESTADÍSTICAS DE PROGRESO (Para la barra visual)
export const obtenerEstadisticasProgreso = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
    ...
    if (!integrante || !integrante.claseId) {
      return res.status(200).json({
        status: "success",
        data: { total: 0, aprobados: 0, porcentaje: 0, clase: "Sin Clase" },
      });
    }
    ...
    const porcentaje =
      totalReq > 0 ? Math.round((aprobados / totalReq) * 100) : 0;
    ...
    return res.status(500).json({
      status: "error",
      message: "Fallo al calcular barra de progreso.",
    });
  }
};

// NUEVO: CARGA MASIVA DE REQUISITOS (Reforzada y Blindada)
export const cargarRequisitosMasivos = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
    ...
    // 🛡️ RBAC Escalonado: Permitimos a REGIONAL y SYSADMIN
    if (rol !== "REGIONAL" && rol !== "SYSADMIN") {
      console.warn(
        `🛑 INTRUSIÓN: Usuario ${req.usuario?.id} con rol ${rol} intentó inyectar manuales.`,
      );
      return res.status(403).json({
        status: "error",
        message:
          "Permisos insuficientes. Operación reservada para Regionales o Sysadmin.",
      });
    }
    ...
    for (const reqItem of requisitos) {
      ...
      const esSub =
        String(reqItem.esSubRequisito).toLowerCase() === "true" ||
        reqItem.esSubRequisito === 1 ||
        reqItem.esSubRequisito === "VERDADERO";
      const reqFoto =
        String(reqItem.requiereFoto).toLowerCase() === "true" ||
        reqItem.requiereFoto === 1 ||
        reqItem.requiereFoto === "VERDADERO";
      ...
      await prisma.requisito.create({
        data: {
          ...
          opcionesExtra: reqItem.opcionesExtra
            ? String(reqItem.opcionesExtra).trim()
            : null,
        },
      });
      contador++;
    ...
    return res.status(201).json({
      status: "success",
      message: `${contador} requisitos inyectados a la base de datos con éxito.`,
    });
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ status: "error", message: "Fallo al procesar la carga masiva." });
  }
};
...
export const limpiarManuales = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 🛡️ REGLA CRÍTICA: Un borrado masivo NUNCA debe estar en manos de un rol operativo.
    if (rol !== "SYSADMIN") {
      console.warn(
        `🛑 ALERTA CRÍTICA: Usuario ${req.usuario?.id} intentó purgar la base de datos de manuales.`,
      );
      return res.status(403).json({
        status: "error",
        message: "Operación clasificada. Exclusivo para SYSADMIN.",
      });
    }
    ...
    return res
      ...
      .json({
        status: "success",
        message: "Base de datos purgada. Manuales limpios.",
      });
  } catch (error) {
    ...
    return res
      ...
      .json({
        status: "error",
        message: "Fallo al intentar limpiar la base de datos.",
      });
  }
};