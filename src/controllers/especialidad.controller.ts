...
export const importarEspecialidadesExcel = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.file) return res.status(400).json({ status: 'error', message: 'No hay archivo adjunto.' });
    ...
    const datosEspecialidades: any[] = XLSX.utils.sheet_to_json(hojaEspecialidades);
    ...
    for (const fila of datosEspecialidades) {
      ...
      const categoriaStr = fila.Categoria ? String(fila.Categoria).trim() : 'Sin Categoría';
      const esVigente = fila.Vigente ? String(fila.Vigente).toUpperCase() !== 'NO' : true;

      await prisma.especialidad.upsert({
        where: { nombre: nombreStr },
        update: { categoria: categoriaStr, vigente: esVigente },
        create: { nombre: nombreStr, categoria: categoriaStr, vigente: esVigente }
      });
      espInsertadas++;
    }
    ...
    if (workbook.SheetNames.length > 1) {
      ...
      for (const fila of datosMaestrias) {
        ...
        const requisitosStr = fila.Requisitos ? String(fila.Requisitos).trim() : '';
    ...
    return res.status(500).json({ status: 'error', message: 'Fallo al procesar el archivo Excel.' });
  }
};

// 2. LECTURA: Traer solo las vigentes (para llenar los menús desplegables del Frontend)
export const obtenerEspecialidadesVigentes = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ status: 'error', message: 'Fallo al leer especialidades.' });
  }
};
...
export const otorgarEspecialidad = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const yaLaTiene = await prisma.integranteEspecialidad.findFirst({
      where: { integranteId: Number(integranteId), especialidadId: Number(especialidadId) }
    });

    if (yaLaTiene) {
      return res.status(400).json({ status: 'error', message: 'El integrante ya posee esta especialidad.' });
    }
    ...
    return res.status(201).json({ status: 'success', message: 'Especialidad otorgada.', data: nuevaAsignacion });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ status: 'error', message: 'Fallo al otorgar especialidad.' });
  }
};
...
export const otorgarMaestria = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const yaLaTiene = await prisma.integranteMaestria.findFirst({
      where: { integranteId: Number(integranteId), maestriaId: Number(maestriaId) }
    });

    if (yaLaTiene) return res.status(400).json({ status: 'error', message: 'El integrante ya posee esta maestría.' });
    ...
    return res.status(201).json({ status: 'success', message: '¡Maestría otorgada con honor!', data: nuevaAsignacion });
  } catch (error) {
